...
let currentUsers :string[]= ["admin", "shoaib", "ali", "ahmad","SAJJAD"];
let newUsers :string[]= ["omar", "umar", "fasih", "ahmad","sajjad"];

for (let i = 0; i < newUsers.length; i++){
    if (currentUsers.includes(newUsers[i])) {
        console.log(`user name ${newUsers[i]} is not available,enter a new user name`)
    } else
    {
        console.log(`user name ${newUsers[i]} is available`)
    }
}